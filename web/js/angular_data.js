...
$scope.user_update_info = function(name,password,mobile) 
	...
	{		
	 ...
	 $http.post('get_worker_info.php',
		{
			'email':$scope.cook_work_email
		})
		.success(function(data, status, headers, config) 
		{
				$scope.workerdetails = data.details;
          });
	...
	$scope.save_update_worker = function() 
	{		
		$http.post('save_update_worker.php',{
		 'name':$scope.cook_name, 'password':$scope.cook_password,
		 'mobile': $scope.cook_mobile, 'email': $scope.cook_work_email})
		.success(function(data, status, headers, config) 
		{
			if(data.success == 1)
			{
				alert("Updated successfully");
				window.location = "worker_update_info.html";
				return;				
			}
			else
			{
...
$scope.share_enquiry = function(email) 
	{
		window.location = "get_enquiry.html";
	...
	$scope.update_enquiry = function() 
	{		
	
		
        $http.post('update_enquiry.php', 
			{	
			'field_1': $scope.field_1, 'field_2':$scope.field_2,
...
$scope.update_status_2 = function(cus_id,field_5,field_6) 
	{
...
$scope.get_rating = function(email) 
	{
		window.location = "view_rating.html";
	...
	$http.post('feedback_get_count.php', {	'email':$scope.cook_work_email})
	.success(function(data, status, headers, config) 
	{
			$scope.feedback_count_details = data.details;
    });
